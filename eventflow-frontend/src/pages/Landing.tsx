...
export default function Landing() {
  ...
  return (
    <div className="min-h-screen bg-[#f4fdff] dark:bg-[#060D1A] text-[#0f172a] dark:text-white font-sans overflow-x-hidden selection:bg-brand selection:text-white relative transition-colors duration-300">
      
      {/* Absolute Background Effects */}
      <div className="fixed inset-0 pointer-events-none overflow-hidden z-0">
        <div className="absolute -top-40 -right-40 w-96 h-96 bg-brand opacity-10 dark:opacity-20 blur-[100px] rounded-full animate-pulse-glow" />
        <div className="absolute top-1/2 -left-20 w-72 h-72 bg-brand-light opacity-10 dark:opacity-10 blur-[80px] rounded-full" />
      </div>

      {/* ── Navbar ── */}
      <nav className="fixed top-0 w-full z-50 flex items-center justify-between px-6 md:px-16 h-20 bg-[#f4fdff]/90 dark:bg-[#060D1A]/90 backdrop-blur-2xl border-b border-brand/10 dark:border-white/10 transition-all duration-300">
        <div className="flex items-center gap-3 group cursor-pointer">
          <svg width="32" height="32" viewBox="0 0 36 36" fill="none" xmlns="http://www.w3.org/2000/svg" className="shrink-0 transition-transform group-hover:scale-105">
            <circle cx="16" cy="16" r="12" stroke="#1F7BF5" strokeWidth="3" fill="none" strokeLinecap="round" strokeDasharray="60 16"></circle>
            <path d="M22 22L30 30" stroke="#1F7BF5" strokeWidth="3" strokeLinecap="round"></path>
            <path d="M14 16L18 20L26 12" stroke="#1F7BF5" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" opacity="0.6"></path>
          </svg>
          <span className="font-extrabold text-2xl tracking-tight text-slate-900 dark:text-white">
            Q<span className="text-[#1F7BF5]">Less</span>
          </span>
        </div>
        <div className="flex items-center gap-4">
          <ThemeToggle />
          <Link to="/login" className="px-6 py-2.5 text-sm font-bold bg-brand text-white rounded-xl hover:opacity-90 hover:shadow-md transition-all duration-300 transform hover:-translate-y-0.5">
            Sign in
          </Link>
        </div>
      </nav>
      ...
      <div className="relative z-10 pt-20">
        ...
        <div className="text-center pt-32 pb-24 px-6 relative">
          ...
          <p className="text-lg md:text-2xl text-slate-500 leading-relaxed max-w-3xl mx-auto mb-14 font-medium fade-in-up" style={{ animationDelay: '0.2s' }}>
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center mb-24 px-4 fade-in-up" style={{ animationDelay: '0.3s' }}>
            <Link to="/login" className="px-10 py-4 text-base font-bold bg-brand text-white rounded-2xl shadow-sm hover:shadow-md transition-all duration-300 transform hover:-translate-y-1">
              Sign in to dashboard
            </Link>
          </div>

          {/* Stats strip */}
          <div className="max-w-4xl mx-auto grid grid-cols-2 md:grid-cols-4 gap-px bg-white border border-slate-200 rounded-3xl overflow-hidden fade-in-up shadow-sm" style={{ animationDelay: '0.4s' }}>
            {STATS.map((s, i) => (
              <div key={s.label} className="p-8 hover:bg-slate-50 transition-colors">
                <div className="text-3xl md:text-4xl font-black text-[#0f172a] tracking-tight mb-2">{s.value}</div>
        ...
        <div className="py-32 px-6 md:px-16 max-w-7xl mx-auto scroll-animate" style={{ opacity: 0 }}>
          <div className="text-center mb-20">
            <h2 className="text-4xl md:text-5xl font-black tracking-tight mb-6 text-[#0f172a]">Four steps. <span style={{ color: 'var(--brand)' }}>Fully automated.</span></h2>
            <p className="text-xl text-slate-500 max-w-2xl mx-auto">From creating the form to scanning the last QR code, everything just works.</p>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6">
            {HOW_IT_WORKS.map((s, i) => (
              <div key={s.n} className="p-8 rounded-[24px] bg-white border border-slate-200 hover:border-brand/50 transition-all duration-300 group hover:-translate-y-2 hover:shadow-md">
                <div className="text-[12px] font-black text-brand bg-brand/10 inline-block px-3 py-1 rounded-full mb-6">{s.n}</div>
                <div className="text-xl font-bold mb-4 text-[#0f172a] group-hover:text-brand transition-colors">{s.title}</div>
        ...
        <div className="py-32 px-6 md:px-16 relative scroll-animate" style={{ opacity: 0 }}>
          <div className="max-w-7xl mx-auto relative">
            <div className="text-center mb-20">
              <h2 className="text-4xl md:text-5xl font-black tracking-tight mb-6 text-[#0f172a]">Everything your club needs</h2>
              <p className="text-xl text-slate-500 max-w-2xl mx-auto">No more stringing together 5 different tools just to run a workshop.</p>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {FEATURES.map(f => (
                <div key={f.title} className="p-8 rounded-[24px] bg-white border border-slate-200 hover:border-brand/40 transition-all duration-300 hover:shadow-md hover:-translate-y-1 group">
                  <div className="w-14 h-14 rounded-2xl bg-slate-50 flex items-center justify-center text-3xl text-brand mb-6 group-hover:scale-110 group-hover:bg-brand group-hover:text-white transition-all duration-300">
                    {f.icon}
                  </div>
                  <div className="text-lg font-bold mb-3 text-[#0f172a]">{f.title}</div>
                  <div className="text-sm text-slate-500 leading-relaxed font-medium">{f.desc}</div>
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="py-32 px-6 max-w-6xl mx-auto scroll-animate" style={{ opacity: 0 }}>
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            <div className="p-10 rounded-[32px] bg-[#f0fbff] border border-brand/20 shadow-sm relative overflow-hidden">
              ...
              <div className="relative z-10">
                ...
                <ul className="space-y-4">
                  {[
                    ...
                    <li key={t} className="flex gap-4 text-[15px] text-[#0f172a] font-medium">
        ...
        {/* ── Footer ── */}
        <div className="border-t border-slate-200 bg-white py-12 px-6 md:px-16 mt-20">
          <div className="max-w-7xl mx-auto flex flex-col md:flex-row items-center justify-between gap-6">
            <div className="flex items-center gap-3">
              <svg width="24" height="24" viewBox="0 0 36 36" fill="none" xmlns="http://www.w3.org/2000/svg">
                <circle cx="16" cy="16" r="12" stroke="var(--brand)" strokeWidth="3" fill="none" strokeLinecap="round" strokeDasharray="60 16"></circle>
                <path d="M22 22L30 30" stroke="var(--brand)" strokeWidth="3" strokeLinecap="round"></path>
                <path d="M14 16L18 20L26 12" stroke="var(--brand)" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" opacity="0.6"></path>
              </svg>
              <span className="font-extrabold text-lg text-[#0f172a]">QLess</span>
            </div>
            <div className="text-sm font-medium text-slate-500">© 2026 QLess · Built for modern clubs</div>
            <div className="flex gap-8">
              {['Privacy', 'Terms', 'Contact'].map(l => (
                <a key={l} href="#" className="text-sm font-bold text-slate-500 hover:text-brand transition-colors">{l}</a>
              ))}
            </div>
          </div>
        </div>
